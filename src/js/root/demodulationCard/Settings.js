...
const Checkdata = {    
    "Frequency": {       
        "range": function (value) {
            return (47000 <= value && value <= 806000) && ValidateUtils.isNumber(value);
        },
        "msg": function(index){
            return `(${index+1})${T('FrequencyError')}[47000KHz,806000KHz].`;
        }
    },
    "SymbolRate": {       
        "range": function (value) {
            return (1000 <= value && value <= 45000) && ValidateUtils.isNumber(value);
        },
        "msg": function(index){
            return `(${index+1})${T('SymbolRateError')}[1000KBaud,45000KBaud].`;
        }
    },
    "port": {
        "range": function (value) {
...
export default class Settings extends React.Component{
    ...
    render(){
        ...
        let modOpt = [{"val":"QAM16","txt":"QAM16"},{"val":"QAM32","txt":"QAM32"}
                    ,{"val":"QAM64","txt":"QAM64"},{"val":"QAM128","txt":"QAM128"}];
        ...
        let cl = [{
            ...
            {
                headerText: `${T('Frequency')}KHz`,
                dataField: 'Frequency',
                dataAlign: 'center',
                rowSpan: 2,
                row: 0,
                dataFormat: (cell, row) => <WVInput {...getProps('Frequency', row)} />,
            },            
            {
                headerText: T('SymbolRate'),
                dataField: 'SymbolRate',
                dataAlign: 'center',
                rowSpan: 2,
                row: 0,
                dataFormat: (cell, row) => <WVInput {...getProps('SymbolRate', row)} />,              
            },
            {
                headerText: T('Constellation'),
                dataField: 'Modulation',
                dataAlign: 'center',
                rowSpan: 2,
                row: 0,
                dataFormat: (cell, row) => <WVSelect {...getProps('Modulation', row, modOpt)}/>,
            },
            {
                headerText: T('Output'),
                dataAlign: 'center',
